...
pub fn part_one(connections: HashMap<String, Vec<String>>) {
    ...
    let total = visit_key(&connections, &mut already_visited, "svr".to_string());
...
pub fn visit_key(
    ...
    if key == "out" {
        if already_visited.contains("fft") && already_visited.contains("dac") {
            return 1;
        }
        return 0;
    }
    ...
    already_visited.insert(key.clone());

    if let Some(neighbors) = connections.get(&key) {
        for value in neighbors {
            total += visit_key(connections, already_visited, value.clone());
        }
    }

    // Backtrack: remove current node from visited set
    already_visited.remove(&key);
